		...
		<div class="card">
			...
			<div class="card-body">
				...
				<div class="d-flex flex-column">
					<a href="{{ route('inscribe', $adoption->id) }}" class="btn btn-primary mt-4 mb-4 fs-2" onclick="return confirm('¿Quieres inscribirte en el proceso de adopción? {{ $adoption->name }} - ID {{ $adoption->id }} ')">Abrázame</a>
					<a href="{{ route('unscribe', $adoption->id) }}" class="btn btn-danger mt-4 mb-4 fs-2" onclick="return confirm('¿Quieres detener este proceso? {{ $adoption->name }} - ID {{ $adoption->id }} ')">Detener inscripción</a>
				</div>
			</div>
		</div>
	</div>	
